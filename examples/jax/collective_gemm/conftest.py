...
def pytest_addoption(parser):
    ...
    parser.addoption("--num-process", action="store", default=1)
...
def distributed_args(request):
    ...
    if request.cls:
        ...
        request.cls.num_processes = int(request.config.getoption("--num-process"))
        ...
        request.cls.num_devices_per_process = 1 if request.cls.local_device_ids is None else len(request.cls.local_device_ids.split(","))
