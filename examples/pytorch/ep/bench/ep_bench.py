...
``nsys`` can attribute kernels. ``--kineto DIR`` dumps a Chrome trace plus a
per-kernel summary table on rank 0.
"""
...
def _parse_args():
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use symm-mem-backed EpBuffer (zero-copy path).",
    )
    p.add_argument(
        "--kineto",
        default=None,
...
def main():
    ...
    if rank == 0:
        print(
            ...
            f"E={E} (local={num_local_experts}) recv_pr={recv_pr} zero_copy={args.zero_copy}"
    ...
    # seed contents from the HBM tensors.
    if args.zero_copy:
        tokens = symm_mem_alloc((T, H), torch.bfloat16, ep_group, device=device)
        tokens.copy_(tokens_hbm)
        topk_w = symm_mem_alloc((T, K), torch.float32, ep_group, device=device)
        topk_w.copy_(topk_w_hbm)
        recv_tokens = buffer.recv_tokens
        recv_w = symm_mem_alloc((recv_pr,), torch.float32, ep_group, device=device)
    else:
        tokens = tokens_hbm
        topk_w = topk_w_hbm
    ...
    torch.cuda.synchronize()
    # fp-equivalent stand-in for an MLP output; symm-mem-backed in zero-copy mode so
    # the raw combine input doesn't get staged.
    if args.zero_copy:
        expert_out = symm_mem_alloc((recv_pr, H), torch.bfloat16, ep_group, device=device)
        expert_out.copy_(recv_tokens)
    else:
