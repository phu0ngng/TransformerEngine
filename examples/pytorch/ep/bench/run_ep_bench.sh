...
#   bash run_ep_bench.sh --zero-copy      # symm-mem-backed EpBuffer
...
NSYS=0; KINETO=0; ZERO_COPY=0; CGRAPH=0
for a in "$@"; do
  case "$a" in
    --nsys)        NSYS=1 ;;
    --kineto)      KINETO=1 ;;
...
# Silence per-call symm-mem fallback warning when running the HBM path.
export NVTE_EP_SILENCE_NONSYMM_WARN="${NVTE_EP_SILENCE_NONSYMM_WARN:-1}"

EXTRA_ARGS=()
TAG="pyt"
[ "${ZERO_COPY}" -eq 1 ] && EXTRA_ARGS+=(--zero-copy) && TAG="${TAG}_zc"
