...
# Paired routing-state pytree returned by ep_prepare and threaded through
# dispatch/combine/*_bwd. handle_mem is the device buffer holding NCCL EP's
# routing metadata (sharded on ep_resource); handle_id is a 1-element int64
# replicated value generated by EPBackend's atomic counter at prepare time —
# it decouples the C++ cache lookup from handle_mem's device pointer (XLA
# may relocate buffers between primitive boundaries).
class EpHandle(NamedTuple):
    handle_mem: jax.Array
    handle_id: jax.Array
...
#          handle_id    [1] int64                      (replicated; cache key for downstream ops)
# Per-shard view at the FFI: token_counts [num_local_experts], handle_mem [N], handle_id [1].
...
class EpPreparePrimitive(BasePrimitive):
    ...
    def abstract(topk_idx_aval, *, dispatch_output_per_expert_alignment, is_outer):
        ...
        # custom_partitioning's sharded_impl): [NLE], [N]. handle_id is [1]
        # in both views (replicated, same uint64_t value on every rank).
        cfg = get_ep_config()
        ep_size = cfg.ep_size
        ...
        handle_id_aval = jax.core.ShapedArray((1,), jnp.int64)
        return token_counts_aval, handle_mem_aval, handle_id_aval
    ...
    def partition(dispatch_output_per_expert_alignment, is_outer, mesh, arg_infos, result_infos):
        ...
        hid_sharding = NamedSharding(mesh, PartitionSpec())  # replicated

        def sharded_impl(topk_idx):
            return EpPreparePrimitive.impl(topk_idx, dispatch_output_per_expert_alignment, False)

        return mesh, sharded_impl, [tc_sharding, hm_sharding, hid_sharding], arg_shardings

    @staticmethod
    def shardy_sharding_rule(*args):
        del args
        return "T topk -> ep_nle, ep_hm, hid"
...
#          handle_id  [1] int64 (replicated; cache key from ep_prepare),
...
class EpDispatchPrimitive(BasePrimitive):
    ...
    impl_static_args = (5, 6, 7)  # recv_capacity, top_k, is_outer
    inner_primitive = None
    outer_primitive = None
    ...
    def abstract(
        ...
        *,
        ...
        del handle_mem_aval, handle_id_aval, topk_idx_aval, topk_weights_aval, top_k
        assert (
    ...
    @staticmethod
    def impl(handle_mem, handle_id, topk_idx, tokens, topk_weights, recv_capacity, top_k, is_outer):
    ...
    def partition(recv_capacity, top_k, is_outer, mesh, arg_infos, result_infos):
        ...
        gsr = global_mesh_resource()
        ep_axis = gsr.ep_resource
        # tokens (arg 3): leading dim must be sharded on dp/fsdp only; hidden replicated.
        tokens_spec = arg_infos[3].sharding.spec
        ...
        def sharded_impl(handle_mem, handle_id, topk_idx, tokens, topk_weights):
            return EpDispatchPrimitive.impl(
                handle_mem, handle_id, topk_idx, tokens, topk_weights, recv_capacity, top_k, False
            )
    ...
    def shardy_sharding_rule(*args):
        del args
        return "ep_hm, hid, T topk_in, T H, T topk -> ep_recv_pr H, ep_recv_pr"
...
def _per_shard_leading(out_leading_shape, resolved_spec, mesh):
    """Per-shard leading shape given resolved partition spec (leading + hidden) and mesh."""
...
class EpCombinePrimitive(BasePrimitive):
    ...
    impl_static_args = (3, 4)  # out_leading_shape, out_partition_spec
    inner_primitive = None
    outer_primitive = None

    @staticmethod
    def abstract(
        handle_mem_aval, handle_id_aval, expert_out_aval, *, out_leading_shape, out_partition_spec
    ):
        del handle_mem_aval, handle_id_aval, out_partition_spec
        assert (
    ...
    @staticmethod
    def lowering(ctx, handle_mem, handle_id, expert_out, *, out_leading_shape, out_partition_spec):
    ...
    @staticmethod
    def impl(handle_mem, handle_id, expert_out, out_leading_shape, out_partition_spec):
    ...
    def partition(out_leading_shape, out_partition_spec, mesh, arg_infos, result_infos):
        ...
        eo_spec = arg_infos[2].sharding.spec
        ...
        def sharded_impl(handle_mem, handle_id, expert_out):
            return EpCombinePrimitive.impl(
                handle_mem, handle_id, expert_out, per_shard_leading, out_partition_spec
            )
    ...
    def shardy_sharding_rule(*args):
        del args
        return "ep_hm, hid, ep_recv_pr H -> dp H"
...
class EpDispatchBwdPrimitive(BasePrimitive):
    ...
    @staticmethod
    def abstract(
        handle_mem_aval, handle_id_aval, grad_aval, *, out_leading_shape, out_partition_spec
    ):
        del handle_mem_aval, handle_id_aval, out_partition_spec
        assert (
    ...
    @staticmethod
    def lowering(ctx, handle_mem, handle_id, grad, *, out_leading_shape, out_partition_spec):
        del out_partition_spec
    ...
    @staticmethod
    def impl(handle_mem, handle_id, grad, out_leading_shape, out_partition_spec):
    ...
    def partition(out_leading_shape, out_partition_spec, mesh, arg_infos, result_infos):
        ...
        gsr = global_mesh_resource()
        ep_axis = gsr.ep_resource
        g_spec = arg_infos[2].sharding.spec
        ...
        def sharded_impl(handle_mem, handle_id, grad):
            return EpDispatchBwdPrimitive.impl(
                handle_mem, handle_id, grad, per_shard_leading, out_partition_spec
            )
...
class EpCombineBwdPrimitive(BasePrimitive):
    ...
    multiple_results = False
    impl_static_args = (3,)  # recv_capacity (GLOBAL = ep_size * recv_capacity_per_rank)
    inner_primitive = None
    outer_primitive = None

    @staticmethod
    def abstract(handle_mem_aval, handle_id_aval, grad_aval, *, recv_capacity):
        del handle_mem_aval, handle_id_aval
        assert (
            len(grad_aval.shape) >= 2
    ...
    @staticmethod
    def lowering(ctx, handle_mem, handle_id, grad, *, recv_capacity):
    ...
    @staticmethod
    def impl(handle_mem, handle_id, grad, recv_capacity):
    ...
    def partition(recv_capacity, mesh, arg_infos, result_infos):
        ...
        def sharded_impl(handle_mem, handle_id, grad):
            return EpCombineBwdPrimitive.impl(handle_mem, handle_id, grad, recv_capacity)
    ...
    def shardy_sharding_rule(*args):
        del args
        return "ep_hm, hid, T H -> ep_recv_pr H"
...
def ep_prepare(topk_idx, dispatch_output_per_expert_alignment=0):
    ...
    Returns (token_counts [num_local_experts] int32,
             EpHandle(handle_mem [N] uint8, handle_id [1] int64) — routing
             state and replicated cache key for downstream ops).
    """
    token_counts, handle_mem, handle_id = EpPreparePrimitive.outer_primitive.bind(
        topk_idx,
        dispatch_output_per_expert_alignment=dispatch_output_per_expert_alignment,
        is_outer=True,
    )
    return token_counts, EpHandle(handle_mem=handle_mem, handle_id=handle_id)


def ep_dispatch_fwd(handle_mem, handle_id, topk_idx, tokens, topk_weights, recv_capacity, top_k):
...
def ep_combine_fwd(handle_mem, handle_id, expert_out, num_local_tokens, out_partition_spec=None):
    ...
    Args:
      handle_id: int64[1] replicated cache key from ep_prepare.
...
def ep_dispatch_bwd(handle_mem, handle_id, grad, num_local_tokens, out_partition_spec=None):
...
def ep_combine_bwd(handle_mem, handle_id, grad, recv_capacity):
