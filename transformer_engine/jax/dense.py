...
@partial(jax.custom_vjp, nondiff_argnums=(3, 6, 7, 8))
def _grouped_dense(
    x,
    kernel,
    group_sizes,
...
def _grouped_dense_fwd_rule(
    ...
    assert x_contracting_dims == (1,) and k_contracting_dims == (1,), (
        "grouped_dense only supports x_contracting_dims=(1,) and k_contracting_dims=(1,) for now, "
    ...
    casted_kernel = tex.batched_quantize(
        kernel,
    ...
    # NN GEMM (M, K) x (G, K, N) -> (M, N) in which M = sum(group_sizes)
    ...
    # TODO(Phuong): check checkpointing of GroupedScaledTensor
    ctx = (
...
def _grouped_dense_bwd_rule(
    ...
    # TODO(Phuong): generalize this for different contracting_dims
