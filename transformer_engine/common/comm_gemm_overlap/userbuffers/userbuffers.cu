...
void userbuffers_sendrecv(const int srchandler, const int dsthandler, const size_t send_offset,
  ...
  void *send_dstptr =
      reinterpret_cast<char *>(comm->get_per_region_peer_ptr(dsthandler)[send_peerlocal]) + send_offset;
...
void userbuffers_sendrecv_multiatomic(const int srchandler, const int dsthandler,
  ...
  int4 *arg4 = reinterpret_cast<int4 *>(comm->get_per_region_peer_ptr(dsthandler)[send_peerlocal]);
  int arg5 = bytes / 16;
...
void userbuffers_recv(const int srchandler, const size_t srcoffset, const int dsthandler,
  ...
  if (comm->push == 0) {
    ...
    void *srcptr = reinterpret_cast<char *>(comm->get_per_region_peer_ptr(srchandler)[peerlocal]) + srcoffset;
