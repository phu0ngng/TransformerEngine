...
void* communicator::get_current_mem_ptr(int region) const {
  ...
  if (is_spmd) {
    ...
    //   return nullptr;
    // }
    NVTE_CHECK(region < NVTE_MAX_REGIONS);
    NVTE_CHECK(current_device < static_cast<int>(per_device_mem_ptr[region].size()));
  ...
  if (comm->use_mc && alloc) {
    ...
    if (comm->nvsize > 1) {
      ...
      NVTE_CHECK_CUDA(cudaMemcpy(
          ...
          comm->peer_ptr[hndl].data(), comm->nvsize * sizeof(void *), cudaMemcpyHostToDevice));
