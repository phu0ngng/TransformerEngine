...
class SpecificGemmPrimitive:
    ...
    @staticmethod
    def infer_sharding_from_operands(operand_shapes, operand_shardings, shape, mesh):
        """
        Infer output sharding for the specific configuration.
        
        Input: [batch, seq, hidden_in] with PartitionSpec(None, tensor, None)
        Weight: [hidden_in, act_num, hidden_out] with PartitionSpec(None, None, tensor)
        Contracting dim: hidden_in (dim 2 of input, dim 0 of weight)
        """
        input_sharding, weight_sharding = operand_shardings
        
        output_sharding = P(None, None, None, None)
        
        print(f"  Inferred output sharding: {output_sharding}")
        return output_sharding
    
    @staticmethod
    def partition(mesh, spec, input_tensor, weight_tensor, contracting_dims):
        """Partition function."""
        print(f"  Partition called with spec: {spec}")
        return SpecificGemmPrimitive.bind(input_tensor, weight_tensor, contracting_dims)
...
def demonstrate_specific_configuration():
    """Demonstrate the specific configuration provided by the user."""
...
if __name__ == "__main__":
    print("Specific GEMM Configuration Demonstration")
    
    demonstrate_specific_configuration()