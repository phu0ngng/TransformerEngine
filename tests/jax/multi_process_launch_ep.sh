...
SCRIPT_NAME="${SCRIPT_NAME:-test_multi_process_ep.py}"
...
if ! grep -qE "Ran [0-9]+ test|^OK$|PASSED" stdout_multi_process.txt; then
  echo "ERROR: rank 0 produced no test summary — likely a hang or early crash."
  echo "      (See stdout_multi_process.txt; was the EP communicator able to init?"
  echo "       NCCL EP HT mode requires NVLS multicast — check 'NVLS multicast support'"
