...
// Token value for (rank, t): (rank * num_tokens + t + 1) / 256.
// Step 1/256 is bf16-exact and yields distinct values across all (rank, t)
// pairs as long as rank * num_tokens + t < 256 — covers ep_size * num_tokens
// up to 256 (e.g. 8 × 32). The previous formula (r+1)*0.01 + t*0.001 had
// exact collisions (e.g. rank=0, t=10 == rank=1, t=0 == 0.020), which
// weakened the recv-multiset check.
...
TEST_F(EPDispatchTest, PrepareAndDispatch) {
  ...
  uint64_t handle_id = nvte_ep_allocate_handle_id(layer_config());
  ASSERT_NO_THROW(nvte_ep_prepare(handle_id, t.topk_idx.tensor, t.token_counts.tensor,
                                  t.handle_mem.tensor, /*alignment=*/0, stream));
  ASSERT_NO_THROW(nvte_ep_dispatch(handle_id, t.handle_mem.tensor, t.topk_idx.tensor,
                                   t.tokens.tensor, t.topk_weights.tensor,
                                   t.recv_tokens.tensor, t.recv_topk_weights.tensor, stream));
...
TEST_F(EPCombineTest, Combine) {
  ...
  ASSERT_NO_THROW(nvte_ep_dispatch(handle_id, t.handle_mem.tensor, t.topk_idx.tensor,
                                   t.tokens.tensor, t.topk_weights.tensor,
                                   t.recv_tokens.tensor, t.recv_topk_weights.tensor, stream));
  ASSERT_NO_THROW(nvte_ep_combine(handle_id, t.handle_mem.tensor, t.recv_tokens.tensor,
...
TEST_F(EPCombineBwdTest, CombineBwdCheck) {
  ...
  ASSERT_NO_THROW(nvte_ep_combine_bwd(handle_id, t.handle_mem.tensor, t.grad_result.tensor,
                                      t.grad_expert.tensor, stream));
  CHECK_CUDA(cudaStreamSynchronize(stream));
...
TEST_F(EPDispatchBwdTest, DispatchBwdCheck) {
  ...
  ASSERT_NO_THROW(nvte_ep_combine_bwd(handle_id, t.handle_mem.tensor, t.grad_result.tensor,
                                      t.grad_expert.tensor, stream));
  ASSERT_NO_THROW(nvte_ep_dispatch_bwd(handle_id, t.handle_mem.tensor, t.grad_expert.tensor,
                                       t.g_recv_topk_weights.tensor, t.grad_tokens.tensor,
                                       t.grad_topk_weights.tensor, stream));
  CHECK_CUDA(cudaStreamSynchronize(stream));
...
TEST_F(EPDispatchBwdGradWeightsTest, RoundTrip) {
  ...
  ASSERT_NO_THROW(nvte_ep_dispatch_bwd(handle_id, t.handle_mem.tensor, t.grad_expert.tensor,
                                       g_recv_t.tensor, t.grad_tokens.tensor,
                                       t.grad_topk_weights.tensor, stream));
  CHECK_CUDA(cudaStreamSynchronize(stream));
...
// Attach a SymmBuf's window+offset to a TensorHandle so the EP backend takes
// the zero-copy ncclEpTensorCreateFromWindow path for this tensor.
static void attach_symm_window(const TensorHandle& h, const SymmBuf& b) {
  nvte_tensor_attach_nccl_window(h.tensor, static_cast<void*>(b.win), /*offset=*/0);
}
...
TEST_F(EPZeroCopyTest, IdentityAllSymm) {
  ...
  uint64_t ref_hid = nvte_ep_allocate_handle_id(layer_config());
  ASSERT_NO_THROW(nvte_ep_prepare(ref_hid, ref_t.topk_idx.tensor, ref_t.token_counts.tensor,
                                  ref_t.handle_mem.tensor, /*alignment=*/0, stream));
  ASSERT_NO_THROW(nvte_ep_dispatch(ref_hid, ref_t.handle_mem.tensor, ref_t.topk_idx.tensor,
                                   ref_t.tokens.tensor, ref_t.topk_weights.tensor,
                                   ref_t.recv_tokens.tensor, ref_t.recv_topk_weights.tensor,
                                   stream));
  ASSERT_NO_THROW(nvte_ep_combine(ref_hid, ref_t.handle_mem.tensor, ref_t.recv_tokens.tensor,
  ...
  attach_symm_window(sym_t.tokens,      sym_tokens);
  attach_symm_window(sym_t.recv_tokens, sym_recv);

  uint64_t sym_hid = nvte_ep_allocate_handle_id(layer_config());
  ASSERT_NO_THROW(nvte_ep_prepare(sym_hid, sym_t.topk_idx.tensor, sym_t.token_counts.tensor,
                                  sym_t.handle_mem.tensor, /*alignment=*/0, stream));
  ASSERT_NO_THROW(nvte_ep_dispatch(sym_hid, sym_t.handle_mem.tensor, sym_t.topk_idx.tensor,
                                   sym_t.tokens.tensor, sym_t.topk_weights.tensor,
                                   sym_t.recv_tokens.tensor, sym_t.recv_topk_weights.tensor,
                                   stream));
  ASSERT_NO_THROW(nvte_ep_combine(sym_hid, sym_t.handle_mem.tensor, sym_t.recv_tokens.tensor,
                                  sym_t.result.tensor, stream));
  CHECK_CUDA(cudaStreamSynchronize(stream));
...
TEST_F(EPZeroCopyTest, IdentityAllSymmRepeated) {
  ...
  attach_symm_window(t.tokens,      sym_tokens);
  attach_symm_window(t.recv_tokens, sym_recv);
...
TEST_F(EPZeroCopyTest, FullPipelineSymm) {
  EPBuffers buf;
  buf.alloc(num_tokens_, top_k_, hidden_dim_, num_local_experts_,
            ep_size_, max_tokens_per_rank_);
  upload_inputs(buf);
  ...
  attach_symm_window(t.tokens,      sym_tokens);
  attach_symm_window(t.recv_tokens, sym_recv);
  attach_symm_window(t.grad_result, sym_grad_result);
  attach_symm_window(t.grad_expert, sym_grad_expert);
