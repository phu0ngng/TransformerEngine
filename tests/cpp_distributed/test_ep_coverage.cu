...
/*
 * Coverage tests for the EP C-API: paths the pipeline suite does not cover.
 *
 *   MultiHandleAllocTest  — allocator returns distinct ids and each works.
 *   TopK1Test             — top_k=1 dispatch/combine/bwd round-trip.
 *   EmptyExpertsTest      — at alignment ∈ {0, 2, 8, 16}, a local expert that
 *                            receives 0 tokens must still produce a correct
 *                            round-trip (covers the SPRINT21 PAD-vs-scan race).
 *   NegativeTests         — alignment mismatch between get_handle_mem_size and
 *                            prepare must throw; null handle_mem must throw.
 */
...
// top1 -> expert 0, top2 -> expert 2 (skipping expert 1). On rank 0's local
// expert list this leaves local-expert 1 empty between two full experts —
// the exact PAD-vs-scan race shape from SPRINT21.
// Requires top_k >= 2 and num_experts >= 3.
...
class EpCoverageBase : public ::testing::Test {
 protected:
  ...
  Bundle make_bundle(int num_tokens, int top_k, int num_local_experts,
    ...
    b.handle_mem_size = nvte_ep_get_handle_mem_size(cfg);
...
TEST_F(MultiHandleAllocTest, IdsAreDistinct) {
  ...
  for (int i = 0; i < kN; ++i) ids[i] = nvte_ep_allocate_handle_id(cfg);
  for (int i = 0; i < kN; ++i) {
...
TEST_F(MultiHandleAllocTest, TwoHandlesCoexist) {
  ...
  NVTEEpLayerConfig cfg{num_local_experts_, top_k, /*alignment=*/0};
  uint64_t id_a = nvte_ep_allocate_handle_id(cfg);
  uint64_t id_b = nvte_ep_allocate_handle_id(cfg);
  ASSERT_NE(id_a, id_b);

  auto run_one = [&](Bundle& x, uint64_t id) {
    ...
    ASSERT_NO_THROW(nvte_ep_prepare(id, topk_idx.tensor, token_counts.tensor,
                                    handle_mem.tensor, /*alignment=*/0, stream));
    ASSERT_NO_THROW(nvte_ep_dispatch(id, handle_mem.tensor, topk_idx.tensor, tokens.tensor,
                                     topk_weights.tensor, recv_tokens.tensor, recv_w.tensor, stream));
    ASSERT_NO_THROW(nvte_ep_combine(id, handle_mem.tensor, recv_tokens.tensor, result.tensor, stream));
  };
  run_one(a, id_a);
  run_one(b, id_b);
...
TEST_F(TopK1Test, RoundTrip) {
  ...
  uint64_t id = nvte_ep_allocate_handle_id({num_local_experts_, top_k, 0});
  ASSERT_NO_THROW(nvte_ep_prepare(id, topk_idx_t.tensor, token_counts_t.tensor,
                                  handle_mem_t.tensor, /*alignment=*/0, stream));
  ASSERT_NO_THROW(nvte_ep_dispatch(id, handle_mem_t.tensor, topk_idx_t.tensor,
                                   tokens_t.tensor, topk_weights_t.tensor,
                                   recv_tokens_t.tensor, recv_w_t.tensor, stream));
  ASSERT_NO_THROW(nvte_ep_combine(id, handle_mem_t.tensor, recv_tokens_t.tensor,
...
TEST_P(EmptyExpertsTest, RoundTripCorrect) {
  ...
  // tokens between two non-empty experts (0 and 2) — SPRINT21 race shape.
  ...
  uint64_t id = nvte_ep_allocate_handle_id({num_local_experts_, top_k, alignment});
  ASSERT_NO_THROW(nvte_ep_prepare(id, topk_idx_t.tensor, token_counts_t.tensor,
                                  handle_mem_t.tensor, alignment, stream));
  ASSERT_NO_THROW(nvte_ep_dispatch(id, handle_mem_t.tensor, topk_idx_t.tensor,
                                   tokens_t.tensor, topk_weights_t.tensor,
                                   recv_tokens_t.tensor, recv_w_t.tensor, stream));
  ASSERT_NO_THROW(nvte_ep_combine(id, handle_mem_t.tensor, recv_tokens_t.tensor,
...
TEST_F(NegativeTests, AlignmentMismatchThrows) {
  ...
  CHECK_CUDA(cudaStreamCreate(&stream));
  uint64_t id = nvte_ep_allocate_handle_id({num_local_experts_, top_k, 0});
  EXPECT_THROW(nvte_ep_prepare(id, topk_idx_t.tensor, token_counts_t.tensor,
                               handle_mem_t.tensor, /*alignment=*/16, stream),
               std::exception);
  CHECK_CUDA(cudaStreamDestroy(stream));
}
...
TEST_F(NegativeTests, NullHandleMemThrows) {
  ...
  uint64_t id = nvte_ep_allocate_handle_id({num_local_experts_, top_k, 0});
  EXPECT_THROW(nvte_ep_prepare(id, topk_idx_t.tensor, token_counts_t.tensor,
                               null_hm_t.tensor, /*alignment=*/0, stream),
               std::exception);
  CHECK_CUDA(cudaStreamDestroy(stream));
}
