...
/*
 * Shared TE EP test infrastructure. All decls are static/inline; include once
 * per TU. Process-level globals are populated by ep_bootstrap(), called from
 * each test binary's main(). Defaults: 4 experts/rank, hidden_dim=256,
...
// View over a caller-owned device buffer. Owns only the NVTETensor metadata,
// not the underlying memory. Move-only.
struct TensorHandle {
...
// RAII owner for a cudaMalloc'd device buffer. Frees on destruction so
// ASSERT_* short-circuits don't leak.
template <typename T>
struct DevBuf {
  T*     ptr   = nullptr;
  size_t count = 0;
...
// Coverage-vs-pipeline parity is enforced by keeping this in one place.
